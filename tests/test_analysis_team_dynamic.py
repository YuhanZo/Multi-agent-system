"""
Mock test for analysis_team dynamic graph (graph_dynamic.py).
Uses Send API for dynamic N worker dispatch.
Runs without research_team by using hardcoded mock state.
"""
import sys
import io
import json
...
from app.agent.analysis_team.graph_dynamic import analysis_team_graph_dynamic
...
MOCK_STATE = {
    ...
    "analysis_tasks": ["extract", "score", "advise"],  # add more tasks here to test N workers
    "analysis_results": [],
    "analysis_report": "",
}

if __name__ == "__main__":
    print("Running dynamic analysis team graph with mock state...")
    print(f"Tasks: {MOCK_STATE['analysis_tasks']}")
    print("-" * 60)

    result = analysis_team_graph_dynamic.invoke(MOCK_STATE)
